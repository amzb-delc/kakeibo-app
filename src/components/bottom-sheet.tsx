...
// --- 背面スクロールロック（モジュールスコープの参照カウント） ---
// シートを重ねると、2つ目以降が「1つ目が適用済みの fixed/top」を現在値として捕捉してしまい、
// 復元時に位置が壊れる（scrollY=0 で固定 → 閉じると先頭へ飛ぶ）。最初のロックでだけ body を
// 固定し、最後のアンロックでだけ元へ戻す参照カウントで防ぐ。
type SavedBodyStyle = {
  position: string;
  top: string;
  left: string;
...
function lockBodyScroll() {
  // 既にロック中（別シートが開いている）なら、現在の固定状態を捕捉せず数えるだけ。
...
function unlockBodyScroll() {
  if (scrollLockCount === 0) return; // 念のため（多重アンロックを無視）
  if (--scrollLockCount > 0) return; // まだ他のシートが開いている
...
export function useBottomSheet(opts?: {
  ...
  // 固定/復元はモジュールスコープの参照カウント（lock/unlockBodyScroll）に委ね、
  // シートを重ねても壊れないようにする。
  useEffect(() => {
    if (!mounted) return;
