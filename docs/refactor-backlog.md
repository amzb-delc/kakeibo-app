...
>
> **状況(2026-06-08): Tier 1〜7 完了でリファクタはひとまず完了。** PR #18〜#33。#7 fetch共通化・T6-2 は意図的に見送り。**Tier 8（スキーマ撤去）は未着手** — DBマイグレ＋設計判断を伴うため、本番化の直前に別途やる方針。
...
| T7-3 | `ExpenseForm`（バリデーション gating＋POST/PATCH 送信→onSuccess→サーバーエラー表示）と `lib/auth.ts`（cookie decode／getDemoUserId キャッシュ）のテスト | `expense-form.test.tsx` / `auth.test.ts` | M | 中 | ✅ |
